...
offset1 = 0 #offset for the diagonal of the NW-SE profile
offset2 = 0 #offset for the diagonal of the NW-SE profile
pref = 1000.
...
shapefile_encoding = 'latin-1' #encoding of the shape files containing boundaries
...
#interpolote <species> to pressure levels and get diagonal, then load pressure values and get diagonal
if len(tarvar.shape) == 3: #for 2d variables
    ...
    tarvar_pres = Ngl.vinth2p(tarvar_flip, acoeff, bcoeff, plevs, pres, intyp=1, p0=pref, ii=1, kxtrp=True) #p0 must be hPa
    ...
    raise Exception('ERROR: invalid number of dimensions for <tarvar>!')
...
for tt in np.arange(tarvar_2d.shape[0]):
    ...
    if domain in {'gal05r', 'gal0504r','gal3'}:
        ...
        mymap.readshapefile(scriptpath+'/shapes/municipios', 'municipios',linewidth=0.35, color='k', antialiased=1, default_encoding=shapefile_encoding)
        mymap.readshapefile(scriptpath+'/shapes/espana', 'espana',linewidth=0.35, color='k', antialiased=1, default_encoding=shapefile_encoding)
        mymap.readshapefile(scriptpath+'/shapes/portugal', 'portugal',linewidth=0.35, color='k', antialiased=1, default_encoding=shapefile_encoding)
    ...
    plt.contourf(Xvprof1,Yvprof1,tarvar_diag1[tt,:,:],cbounds,cmap=colormap,vmin=cbounds[0],vmax=cbounds[-1])
    plt.fill_between(lon_diag1,pres[tt,:,:].diagonal(offset=offset1,axis1=0,axis2=1)/100.,y2=pref,color='grey') #plot first model level to mimic surface orography
    #figure finetuning
    ...
    plt.contourf(Xvprof2,Yvprof2,tarvar_diag2[tt,:,:],cbounds,cmap=colormap,vmin=cbounds[0],vmax=cbounds[-1])
    plt.fill_between(lon_diag2,np.fliplr(pres[tt,:,:]).diagonal(offset=offset2,axis1=0,axis2=1)/100.,y2=pref,color='grey') #plot first model level to mimic surface orography
    #figure finetuning
    plt.ylim(plevs[0],plevs[-1])
