...
tardate='20190624' #tested for global CAMS files only!
tarhour='00'
datatype = 'test' #test or prod

#set environmental variables
store = os.getenv('STORE_METEO')
lustre = os.getenv('LUSTRE')

#dimensions of the CAMS input files are (latitude: 451, level: 60, longitude: 900, time: 11)
source = store+'/OP/DATOS/CICC/CAMS_137/'+tardate+'/'+tarhour
source_clim = store+'/OP/DATOS/CICC/chimere2017r4/MACC/clim_global'
outpath = lustre+'/OP/DATOS/CICC/chimere2017r4/MACC/myclim_regional_137'
months=('01','02','03','04','05','06','07','08','09','10','11','12')
#months=('01','02')
#variables = ('BIGALK','BIGENE') #set variable name as defined in CAMS
variables = ('BIGALK','BIGENE','CH3CHO','GLYOXAL','H2O2','NH3','TOLUENE','C10H16','TEMP','SS1','SS2','SS3')
execfile('model_densities.py')

### EXECUTE #############################################################
for ii in xrange(len(variables)):
    #load file in cache
    tarvar = variables[ii]
    srcfile = source+'/z_cams_c_ecmf_'+tardate+tarhour+'0000_'+datatype+'_fc_ml_*_go3.nc' #all forecast hours of an example 3d varible, needed to define the target dates, latitudes and longitudes    
    ...
    lons_ind_east = np.where(lons <= 180)
    lons_ind = np.squeeze(np.concatenate((lons_ind_west,lons_ind_east),axis=1))
    lons = nc.variables['longitude'][lons_ind]    
    lons = lons.values
    lons_aux = np.copy(lons)
    aux_ind = np.where(lons_aux>180)
    #used for interpolation, where basemap function needs longitudes in ascending order
    lons_aux[aux_ind] = lons_aux[aux_ind]-360
    ...
    for mm in xrange(len(months)):      
        ...
        lons_ind_east_clim = np.where(lons_clim <= 180)
        ...
        lons_clim = lons_clim.values        
        lons_aux_clim = np.copy(lons_clim)
        aux_ind_clim = np.where(lons_aux_clim>180)
        lons_aux_clim[aux_ind_clim] = lons_aux_clim[aux_ind_clim]-360 #used for interpolation, where basemap function needs longitudes in ascending order
        data_clim = nc_clim.variables[tarvar][:,:,lats_ind_clim,lons_ind_clim]
        data_clim = np.squeeze(data_clim.values)
        ...
        lats_shift = lats[lats_ind_org]
        
        #cut out target region
        cutlons_ind=np.squeeze(np.array(np.where((lons_shift >= 340) | (lons_shift <= 10)))) #must be identical to regional cut of the CAMS data achieved by NCO or CDO
        cutlats_ind=np.squeeze(np.array(np.where((lats_shift >= 30) & (lats_shift <= 60)))) #  dito
        lons_end = lons_shift[cutlons_ind]
        lats_end = lats_shift[cutlats_ind]
        data_out = data_out[:,cutlats_ind,:]
        data_out = data_out[:,:,cutlons_ind]
        
        #then interpolate vertically from 60 to 137 levels
        data_out_137 = np.zeros((len(density_137), data_out.shape[1], data_out.shape[2]))
        idx60_in_idx137 = density_60.searchsorted(density_137)
        data_out_137[idx60_in_idx137,] = data_out[idx60_in_idx137,]
        ...
        output = xr.DataArray(data_out_137, coords=[levs, np.double(lats_end), np.double(lons_end)], dims=['lev', 'lat', 'lon'], name=variables[ii])
