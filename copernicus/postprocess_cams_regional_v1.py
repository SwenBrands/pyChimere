...
source_clim = cdir+'/myclim_regional'
...
for ii in xrange(len(variables_in)):
    ...
    #rename filenamevar in case of ch4. From 20180710 onwards, 'ch4' was changed to 'nc4_c' in the filename label
    if int(tardate) > int('20180709'):
        if srcvar == 'ch4':
            print('INFO: filename label is renamed from ch4 to ch4_c...')
            filenamevar = 'ch4_c'
        else:
            filenamevar = srcvar
    else:
        print('INFO: no need to rename CH4 variable...')
        filenamevar = srcvar
    
    tarvar = variables_out[ii]     
    srcfile = source+'/z_cams_c_ecmf_'+tardate+tarhour+'0000_prod_fc_ml_*_'+filenamevar+'.nc'
    ...
    elif tarvar in ('DUST1','DUST2','DUST3','OM','BC','SO4'):
        #this a 4D variable
