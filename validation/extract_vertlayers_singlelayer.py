...
max_leadtime = int(24) #retain temporal entries until time step with this index
...
times = nc.Times.values[0:max_leadtime,]
lats = nc.lat.values
lons = nc.lon.values
#load variables in a loop
for vv in range(len(outvars)):
    if outvars[vv] == 'hght':
        variable = nc.variables[outvars[vv]][0:max_leadtime,]
        data = variable.values[0:max_leadtime,]
    else:
        variable = nc.variables[outvars[vv]][:,level,:,:][0:max_leadtime,]
        data = np.squeeze(variable.values,axis=1)[0:max_leadtime,]
