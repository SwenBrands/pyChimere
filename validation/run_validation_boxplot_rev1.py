...
## DEFINE USER OPTIONS
savepath = lustre+'/SWEN/publications/2018_Brands_chimere/figs/boxplots/'+stationtype
...
variables = ['NO2','O3','PM25','PM10','SO2']
...
#xlabel = ['CS10H', 'CS10E', 'CM10H', 'CM10E', 'CS20H', 'CS20E', 'CM20H', 'CM20E', 'FS10H', 'FS10E', 'FM10H', 'FM10E', 'FS20H', 'FS20E','FM20H', 'FM20E'] #CM = "coarse meteorology, NBE = no biogenic emissions"
xlabel = ['CS10E', 'CM10E', 'CS20E', 'CM20E', 'FS10E', 'FM10E', 'FS20E', 'FS20ER', 'FS20ER14', 'FM20E', 'FM20ER14', 'CS10H', 'CM10H', 'CS20H', 'CM20H', 'FS10H', 'FM10H', 'FS20H', 'FM20H', 'FM20H-P', 'FM20H-C', 'FM20H-N', 'IM20E18', 'IM20myE'] #CM = "coarse meteorology, NBE = no biogenic emissions"
...
titlesize = 5
labelsize = 4
biastype = 'percent' #absolute or percent
plot_whiskers = 'yes' #yes or no
...
bestscore_width = 1
showmeans = False
whis_factor = 1. #for muliplication with IQR, used to plot the separation lines between experiments
...
src_dirs = [lustre+'/SWEN/DATA/OBSDATA', RESpath+'/jja_24/validation', RESpath+'/jja_23/validation', RESpath+'/jja_22/validation', RESpath+'/jja_21/validation', \
            RESpath+'/jja_20/validation', RESpath+'/jja_19/validation', RESpath+'/jja_18/validation', RESpath+'/jja_26/validation', RESpath+'/jja_27/validation', RESpath+'/jja_17/validation', \
            RESpath+'/jja_28/validation', RESpath+'/jja_13_gal15r/validation', RESpath+'/jja_10_gal15r/validation', RESpath+'/jja_12_gal15r/validation', RESpath+'/jja_02_gal15r/validation', \
            RESpath+'/jja_13/validation', RESpath+'/jja_10/validation', RESpath+'/jja_12/validation', RESpath+'/jja_08/validation', RESpath+'/jja_09/validation', \
            RESpath+'/jja_14/validation', RESpath+'/jja_15/validation', RESpath+'/jja_25/validation', RESpath+'/jja_30/validation']
...
#check for existence of target directory and create it if necessary
...
#define ytick positions
ypos = range(1,len(xlabel)+1)
...
for vv in range(len(variables)):
    ...
    #plot the results, note that BIAS, RHO, RATIO and MAE_SKILL are flipped in this section if len(station_names) >= nrstations!!!
    fig, axs = plt.subplots(nr_rows, nr_cols, squeeze=False)
    #plt.setp(axs, yticks=range(1,len(xlabel)+1), yticklabels=xlabel)
    ...
    if len(station_names) < nrstations:
        ...
        boxout = axs[0][0].boxplot(BIAS, vert=False, flierprops=flierprops, showcaps=showcaps, showfliers=showfliers, showmeans=showmeans)
        #axs[0][0].set_xlim(xlim_bias)
        axs[0][0].plot([0,0],[0,MODARR.shape[1]+2],'r',lw=bestscore_width )
        PCT25 = np.percentile(BIAS,25,axis=0)
        PCT75 = np.percentile(BIAS,75,axis=0)
        IQR = PCT75-PCT25
        lowerlim = np.min(PCT25 - IQR*whis_factor)
        upperlim = np.max(PCT75 + IQR*whis_factor)
        if plot_whiskers == 'yes':
            axs[0][0].plot([lowerlim, upperlim],[separator1,separator1],'k', lw=sep1_width, ls='--' )
            axs[0][0].plot([lowerlim, upperlim],[separator2,separator2],'k', lw=sep2_width, ls='--' )
        plt.sca(axs[0, 0])
        plt.yticks(ypos, np.flip(xlabel), color='black')
        plt.xlabel('BIAS (%)')
        
        
    retainind=np.where(~np.any(np.isnan(RHO),axis=0))[0]
    RHO = np.rot90(RHO[:,retainind])
    legend_rho = [station_names[ii] for ii in retainind]
    if len(station_names) < nrstations:
        axs[1][0].plot([0.5,MODARR.shape[1]+0.5],[0,0],'r')
        X = np.tile(range(MODARR.shape[1]),(len(station_names),1))
        axs[1][0].plot(X+1, RHO, linestyle='None', marker=markersymbol)
    else:
        RHO = np.flip(RHO, axis=1)
        explabels = np.repeat(xlabel,len(retainind))
        horeslabels = np.repeat(hores,len(retainind)) 
        layerslabels = np.repeat(layers,len(retainind)) 
        axs[1][0].boxplot(RHO, vert=False, flierprops=flierprops, showcaps=showcaps, showfliers=showfliers, showmeans=showmeans)
        #axs[0][1].set_xlim(xlim_rho)
        #axs[1][0].plot([0,0],[0,MODARR.shape[1]+2],'r',lw=bestscore_width)
        PCT25 = np.percentile(RHO,25,axis=0)
        PCT75 = np.percentile(RHO,75,axis=0)
        IQR = PCT75-PCT25
        lowerlim = np.min(PCT25 - IQR*whis_factor)
        upperlim = np.max(PCT75 + IQR*whis_factor)
        if plot_whiskers == 'yes':
            axs[1][0].plot([lowerlim, upperlim],[separator1,separator1],'k', lw=sep1_width, ls='--' )
            axs[1][0].plot([lowerlim, upperlim],[separator2,separator2],'k', lw=sep2_width, ls='--' )
        plt.sca(axs[1, 0])
        plt.yticks(ypos, np.flip(xlabel), color='black')
        plt.xlabel('R')
    ...
    RATIO = np.rot90(RATIO[:,retainind])
    legend_ratio = [station_names[ii] for ii in retainind]
    if len(station_names) < nrstations:
        axs[0][1].plot([0.5,MODARR.shape[1]+0.5],[0,0],'r')
        X = np.tile(range(MODARR.shape[1]),(len(station_names),1))
        axs[0][1].plot(X+1, RATIO, linestyle='None', marker=markersymbol)        
    else:
        RATIO = np.flip(RATIO, axis=1)
        explabels = np.repeat(xlabel,len(retainind))
        horeslabels = np.repeat(hores,len(retainind)) 
        layerslabels = np.repeat(layers,len(retainind)) 
        axs[0][1].boxplot(RATIO, vert=False, flierprops=flierprops, showcaps=showcaps, showfliers=showfliers, showmeans=showmeans)
        #axs[1][0].set_xlim(xlim_ratio)
        axs[0][1].plot([1,1],[0,MODARR.shape[1]+2],'r',lw=bestscore_width)
        PCT25 = np.percentile(RATIO,25,axis=0)
        PCT75 = np.percentile(RATIO,75,axis=0)
        IQR = PCT75-PCT25
        lowerlim = np.min(PCT25 - IQR*whis_factor)
        upperlim = np.max(PCT75 + IQR*whis_factor)
        if plot_whiskers == 'yes':
            axs[0][1].plot([lowerlim, upperlim],[separator1,separator1],'k', lw=sep1_width, ls='--' )
            axs[0][1].plot([lowerlim, upperlim],[separator2,separator2],'k', lw=sep2_width, ls='--' )
        plt.sca(axs[0, 1])
        plt.yticks(ypos, np.flip(xlabel), color='black')
        plt.xlabel('RATIO')
    ...
    MAE_SKILL = np.rot90(MAE_SKILL[:,retainind])
    legend_mae = [station_names[ii] for ii in retainind]
    if len(station_names) < nrstations:
        axs[1][1].plot([0.5,MODARR.shape[1]+0.5],[0,0],'r',lw=bestscore_width)
        X = np.tile(range(MODARR.shape[1]),(len(station_names),1))
        axs[1][1].plot(X+1, MAE_SKILL, linestyle='None', marker=markersymbol)        
    else:
        MAE_SKILL = np.flip(MAE_SKILL, axis=1)
        explabels = np.repeat(xlabel,len(retainind))
        horeslabels = np.repeat(hores,len(retainind)) 
        layerslabels = np.repeat(layers,len(retainind)) 
        axs[1][1].boxplot(MAE_SKILL, vert=False, flierprops=flierprops, showcaps=showcaps, showfliers=showfliers, showmeans=showmeans)
        #axs[1][1].set_xlim(xlim_skill)
        axs[1][1].plot([0,0],[0,MODARR.shape[1]+2],'r')
        PCT25 = np.percentile(MAE_SKILL,25,axis=0)
        PCT75 = np.percentile(MAE_SKILL,75,axis=0)
        IQR = PCT75-PCT25
        lowerlim = np.min(PCT25 - IQR*whis_factor)
        upperlim = np.max(PCT75 + IQR*whis_factor)
        if plot_whiskers == 'yes':
            axs[1][1].plot([lowerlim, upperlim],[separator1,separator1],'k', lw=sep1_width, ls='--' )
            axs[1][1].plot([lowerlim, upperlim],[separator2,separator2],'k', lw=sep2_width, ls='--' )
        plt.sca(axs[1, 1])
        plt.yticks(ypos, np.flip(xlabel), color='black')
        plt.xlabel('MAESS (%)')
    
    fig.savefig(savepath+'/boxplot_'+valmode+'_'+variables[vv]+'_'+aggval+'_'+stationtype+'_'+biastype+'bias'+extension, dpi=300)
    plt.close('all')
...
for iii in ['MAE','HIMAE_obs','LOMAE_obs','HIMAE_mod','LOMAE_mod']:
    ...
    fig.savefig(savepath+'/pcolor_'+iii+'_'+valmode+'_'+aggval+'_'+stationtype+extension, dpi=300)
    plt.close('all')

print('INFO: run_validation_boxplot_rev1.py ended successfully!')
