...
variables = ['PM25','O3','PM10','NO2']
#xlabel = ['TR', 'OP', 'HTAP', 'HTAPC']
...
xlim_bias = [-80,80] #in percent, [-80,80] for max O3, PM
xlim_rho = [-0.1,1] #[-0.1,1] for max O3, PM
xlim_ratio = [0,1.6] #[0,1.6] for max O3, PM
xlim_skill = [-45,45] #[-45,45] for max O3, PM 
xlim_skill_ext = [-60,60] #[-60,60] for max O3, PM
box_linewidth = 1
showcaps=False
...
#src_dirs = [lustre+'/SWEN/DATA/OBSDATA', lustre+'/SWEN/DATA/MODELDATA', RESpath+'/jja_05/validation', RESpath+'/jja_06/validation', RESpath+'/jja_06_corr_gal3/validation']
src_dirs = [lustre+'/SWEN/DATA/OBSDATA', RESpath+'/jja_13_gal15r/validation', RESpath+'/jja_10_gal15r/validation', RESpath+'/jja_12_gal15r/validation', \
            RESpath+'/jja_02_gal15r/validation', RESpath+'/jja_13/validation', RESpath+'/jja_10/validation', RESpath+'/jja_12/validation', RESpath+'/jja_08/validation']
...
for vv in range(len(variables)):
    ...
    if len(station_names) < nrstations:
        ...
        subplot = sns.catplot(kind=kind, y='experiment', x='bias', data=BIASpd, hue=groupby, orient='h', legend=legend, fliersize=fliersize, ax = axs[0][0], showcaps=showcaps)
    ...
    if len(station_names) < nrstations:
        ...
        X = np.transpose(np.tile(range(MODARR.shape[1]),(len(station_names),1)))
        axs[2][1].plot(X+1, MAE_SKILL, linestyle='None', marker=markersymbol)        
    else:
        explabels = np.repeat(xlabel,len(retainind))
        horeslabels = np.repeat(hores,len(retainind)) 
        layerslabels = np.repeat(layers,len(retainind)) 
        MAE_SKILLpd = pd.DataFrame(dict(mae_skill=MAE_SKILL.ravel(),experiment=explabels,hores=horeslabels,layers=layerslabels))
        sns.catplot(kind=kind, y='experiment', x='mae_skill', data=MAE_SKILLpd, hue=groupby, orient='h', legend=legend, fliersize=fliersize, ax = axs[2][1], showcaps=showcaps)
        #axs[2][1].set_xlim(xlim_skill)
        axs[2][1].plot([0,0],[0,MODARR.shape[1]+2],'r')
    
    if aggval in ('mean','max','hourly'):
        ...
        if len(station_names) < nrstations:
            ...
            X = np.transpose(np.tile(range(MODARR.shape[1]),(len(station_names),1)))
            axs[1][1].plot(X+1, HIBIAS_obs, linestyle='None', marker=markersymbol)        
        else:
            explabels = np.repeat(xlabel,len(retainind))
            horeslabels = np.repeat(hores,len(retainind)) 
            layerslabels = np.repeat(layers,len(retainind)) 
            HIBIAS_obs_pd = pd.DataFrame(dict(hibias_obs=HIBIAS_obs.ravel(),experiment=explabels,hores=horeslabels,layers=layerslabels))
            sns.catplot(kind=kind, y='experiment', x='hibias_obs', data=HIBIAS_obs_pd, hue=groupby, orient='h', legend=legend, fliersize=fliersize, ax = axs[1][1], showcaps=showcaps)
        ...
        if len(station_names) < nrstations:
            ...
            X = np.transpose(np.tile(range(MODARR.shape[1]),(len(station_names),1)))
            axs[2][0].plot(X+1, HIBIAS_mod, linestyle='None', marker=markersymbol)
        else:
            explabels = np.repeat(xlabel,len(retainind))
            horeslabels = np.repeat(hores,len(retainind)) 
            layerslabels = np.repeat(layers,len(retainind)) 
            HIBIAS_mod_pd = pd.DataFrame(dict(hibias_mod=HIBIAS_mod.ravel(),experiment=explabels,hores=horeslabels,layers=layerslabels))
            sns.catplot(kind=kind, y='experiment', x='hibias_mod', data=HIBIAS_mod_pd, hue=groupby, orient='h', legend=legend, fliersize=fliersize, ax = axs[2][0], showcaps=showcaps)
            axs[2][0].plot([0,0],[0,MODARR.shape[1]+2],'r')
        
        retainind=np.where(~np.any(np.isnan(HIMAE_obs_SKILL),axis=0))[0]
        HIMAE_obs_SKILL = HIMAE_obs_SKILL[:,retainind]       
        if len(station_names) < nrstations:
            #axs[3][0].plot([0.5,MODARR.shape[1]+0.5],[0,0],'r')
            X = np.transpose(np.tile(range(MODARR.shape[1]),(len(station_names),1)))
            axs[3][0].plot(X+1, HIMAE_obs_SKILL, linestyle='None', marker=markersymbol)        
        else:
            explabels = np.repeat(xlabel,len(retainind))
            horeslabels = np.repeat(hores,len(retainind)) 
            layerslabels = np.repeat(layers,len(retainind)) 
            HIMAE_obs_SKILLpd = pd.DataFrame(dict(himae_obs_skill=HIMAE_obs_SKILL.ravel(),experiment=explabels,hores=horeslabels,layers=layerslabels))
            sns.catplot(kind=kind, y='experiment', x='himae_obs_skill', data=HIMAE_obs_SKILLpd, hue=groupby, orient='h', legend=legend, fliersize=fliersize, ax = axs[3][0], showcaps=showcaps)
            #axs[3][0].set_xlim(xlim_skill_ext)
            axs[3][0].plot([0,0],[0,MODARR.shape[1]+2],'r')
        
        retainind=np.where(~np.any(np.isnan(HIMAE_mod_SKILL),axis=0))[0]
        HIMAE_mod_SKILL = HIMAE_mod_SKILL[:,retainind]
        if len(station_names) < nrstations:
            #axs[3][1].plot([0.5,MODARR.shape[1]+0.5],[0,0],'r')
            X = np.transpose(np.tile(range(MODARR.shape[1]),(len(station_names),1)))
            axs[3][1].plot(X+1, HIMAE_mod_SKILL, linestyle='None', marker=markersymbol)        
        else:
            explabels = np.repeat(xlabel,len(retainind))
            horeslabels = np.repeat(hores,len(retainind)) 
            layerslabels = np.repeat(layers,len(retainind)) 
            HIMAE_mod_SKILLpd = pd.DataFrame(dict(himae_mod_skill=HIMAE_mod_SKILL.ravel(),experiment=explabels,hores=horeslabels,layers=layerslabels))
            sns.catplot(kind=kind, y='experiment', x='himae_mod_skill', data=HIMAE_mod_SKILLpd, hue=groupby, orient='h', legend=legend, fliersize=fliersize, ax = axs[3][1], showcaps=showcaps)
            #axs[3][1].set_xlim(xlim_skill_ext)
            axs[3][1].plot([0,0],[0,MODARR.shape[1]+2],'r')
        ...
        if len(station_names) < nrstations:
            #axs[1][1].plot([0.5,MODARR.shape[1]+0.5],[0,0],'r')
            X = np.transpose(np.tile(range(MODARR.shape[1]),(len(station_names),1)))
            axs[1][1].plot(X+1, LOBIAS_obs, linestyle='None', marker=markersymbol)        
        else:
            explabels = np.repeat(xlabel,len(retainind))
            horeslabels = np.repeat(hores,len(retainind)) 
            layerslabels = np.repeat(layers,len(retainind)) 
            LOBIAS_obs_pd = pd.DataFrame(dict(lobias_obs=LOBIAS_obs.ravel(),experiment=explabels,hores=horeslabels,layers=layerslabels))
            sns.catplot(kind=kind, y='experiment', x='lobias_obs', data=LOBIAS_obs_pd, hue=groupby, orient='h', legend=legend, fliersize=fliersize, ax = axs[1][1], showcaps=showcaps)
            axs[1][1].plot([0,0],[0,MODARR.shape[1]+2],'r')
        ...
        if len(station_names) < nrstations:
            #axs[2][0].plot([0.5,MODARR.shape[1]+0.5],[0,0],'r')
            X = np.transpose(np.tile(range(MODARR.shape[1]),(len(station_names),1)))
            axs[2][0].plot(X+1, LOBIAS_mod, linestyle='None', marker=markersymbol)        
        else:
            explabels = np.repeat(xlabel,len(retainind))
            horeslabels = np.repeat(hores,len(retainind)) 
            layerslabels = np.repeat(layers,len(retainind)) 
            LOBIAS_mod_pd = pd.DataFrame(dict(lobias_mod=LOBIAS_mod.ravel(),experiment=explabels,hores=horeslabels,layers=layerslabels))
            sns.catplot(kind=kind, y='experiment', x='lobias_mod', data=LOBIAS_mod_pd, hue=groupby, orient='h', legend=legend, fliersize=fliersize, ax = axs[2][0], showcaps=showcaps)
            axs[2][0].plot([0,0],[0,MODARR.shape[1]+2],'r')
            
        retainind=np.where(~np.any(np.isnan(LOMAE_obs_SKILL),axis=0))[0]
        LOMAE_obs_SKILL = LOMAE_obs_SKILL[:,retainind]
        if len(station_names) < nrstations:
            #axs[3][0].plot([0.5,MODARR.shape[1]+0.5],[0,0],'r')
            X = np.transpose(np.tile(range(MODARR.shape[1]),(len(station_names),1)))
            axs[3][0].plot(X+1, LOMAE_obs_SKILL, linestyle='None', marker=markersymbol)        
        else:
            explabels = np.repeat(xlabel,len(retainind))
            horeslabels = np.repeat(hores,len(retainind)) 
            layerslabels = np.repeat(layers,len(retainind)) 
            LOMAE_obs_SKILLpd = pd.DataFrame(dict(lomae_obs_skill=LOMAE_obs_SKILL.ravel(),experiment=explabels,hores=horeslabels,layers=layerslabels))
            sns.catplot(kind=kind, y='experiment', x='lomae_obs_skill', data=LOMAE_obs_SKILLpd, hue=groupby, orient='h', legend=legend, fliersize=fliersize, ax = axs[3][0], showcaps=showcaps)
            #axs[3][0].set_xlim(xlim_skill_ext)
            axs[3][0].plot([0,0],[0,MODARR.shape[1]+2],'r')
        
        retainind=np.where(~np.any(np.isnan(LOMAE_mod_SKILL),axis=0))[0]
        LOMAE_mod_SKILL = LOMAE_mod_SKILL[:,retainind]
        if len(station_names) < nrstations:
            #axs[3][1].plot([0.5,MODARR.shape[1]+0.5],[0,0],'r')
            X = np.transpose(np.tile(range(MODARR.shape[1]),(len(station_names),1)))
            axs[3][1].plot(X+1, LOMAE_mod_SKILL, linestyle='None', marker=markersymbol)        
        else:
            explabels = np.repeat(xlabel,len(retainind))
            horeslabels = np.repeat(hores,len(retainind)) 
            layerslabels = np.repeat(layers,len(retainind)) 
            LOMAE_mod_SKILLpd = pd.DataFrame(dict(lomae_mod_skill=LOMAE_mod_SKILL.ravel(),experiment=explabels,hores=horeslabels,layers=layerslabels))
            sns.catplot(kind=kind, y='experiment', x='lomae_mod_skill', data=LOMAE_mod_SKILLpd, hue=groupby, orient='h', legend=legend, fliersize=fliersize, ax = axs[3][1], showcaps=showcaps)
            #axs[3][1].set_xlim(xlim_skill_ext)
            axs[3][1].plot([0,0],[0,MODARR.shape[1]+2],'r')
    else:
        raise Exception('check entry for <aggval>!')
...
cblim_himae_obs = 25 #25 for nanmedian
