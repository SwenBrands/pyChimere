...
levels=[0,1] #model levels to be retained
...
if chemtype == 'melchior_low':
    outvars = ['O3','NO2','NO','PAN','HNO3','H2O2','HONO','SO2','CO','OH','NOX','TOTPAN','NOY','HOX','ROX','ANT','BIO','COVS','ANTS','OX','pDUST','pBCAR','pOCAR', \
            ...
            'airm','relh','temp','winz','winm','winw','sphu','kzzz','clwc','cliq','cice','pres','dpeu','dped','dpdu','dpdd','flxu','flxd','jO3','jNO2']
elif chemtype == 'melchior_low_chemonly':
    outvars = ['O3','NO2','NO','PAN','HNO3','H2O2','HONO','SO2','CO','OH','NOX','TOTPAN','NOY','HOX','ROX','ANT','BIO','COVS','ANTS','OX','pDUST','pBCAR','pOCAR', \
            'pPPM','pSALT','pAnA1D','pAnBmP','pBiA1D','pBiBmP','pISOPA1','pH2SO4','pHNO3','pNH3','pWATER','PM25','PM25bio','PM25ant','PM10','PM10bio','PM10ant']
...
bottom_top = nc.bottom_top[levels].values
times = nc.Times.values
lats = nc.lat.values
lons = nc.lon.values
#load variables in a loop
for vv in range(len(outvars)):
    variable = nc.variables[outvars[vv]][:,levels,:,:]
    data = variable.values
    xrarr = xr.DataArray(data, coords=[times, bottom_top, np.unique(lats), np.unique(lons)], dims=['Times', 'bottom_top', 'south_north', 'west_east'], attrs=variable.attrs, name=outvars[vv])
